...
<head>
    ...
    <!-- CORREÇÃO 1: Uso do BASE_DIR garante que o CSS carregue em qualquer rota -->
...
<body class="bg-surface text-on-surface font-body flex flex-col min-h-screen">
    ...
    <main class="flex-1 pt-32 pb-20 px-6 max-w-[80rem] mx-auto w-full">
        <div class="flex justify-between items-end mb-12">
            ...
            <!-- CORREÇÃO 2: Links internos usando BASE_DIR -->
            <a href="<?= BASE_DIR ?>/loja/carrinho" class="p-3 bg-surface-container border border-outline-variant/50 rounded hover:border-primary transition-colors flex items-center gap-2 no-underline">
                <span class="material-symbols-outlined text-primary">shopping_bag</span>
                <span class="text-xs font-bold uppercase tracking-wider hidden md:block">Ver Bolsa</span>
            </a>
        </div>

        <!-- COMPONENTE GLOBAL DE ALERTAS (Substitui o código repetido) -->
                ...
                <div class="bg-surface-container border border-outline-variant/40 rounded-sm overflow-hidden flex flex-col group hover:-translate-y-1 transition-all hover:shadow-[0_16px_40px_rgba(0,0,0,0.4)]">
                    ...
                    <div class="h-48 relative bg-surface-container-lowest flex items-center justify-center overflow-hidden">
                        <?php if(!empty($produto['imagem'])): ?>
                            <!-- CORREÇÃO 3: Caminho completo para a pasta Uploads onde a imagem foi salva -->
                            <img src="<?= BASE_DIR ?>/Public/Uploads/<?= htmlspecialchars($produto['imagem']) ?>" alt="<?= htmlspecialchars($produto['nome']) ?>" class="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity duration-500">
                        <?php else: ?>
